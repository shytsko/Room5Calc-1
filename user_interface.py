...
def calc_comp(count=pos, msg='Complex numbers'):
    try:
        ...
        if mathExp.upper() == 'Q' or mathExp.upper() == 'Й':
            ...
            print(
                'Работа программы с комплексными числами: \033[32m(2+3j)*(6+12.3j)\033[0m \nОтвет: \033[36m(2+3j)*(6+12.3j) = (-24.900000000000006+42.6j)\033[0m')
            calc_comp()
        elif mathExp.upper() == 'E' or mathExp.upper() == 'У':
            return
        else:
...
def calc_real(count=pos, msg='Rational numbers'):
    try:
        ...
        if mathExp.upper() == 'Q' or mathExp.upper() == 'Й':
            ...
            print(
                'Работа программы с рациональными числами:\033[32m (1+2)*4+3\033[0m\nОтвет: \033[36m(1+2)*4+3= 15.0\033[0m')
            calc_real()
        elif mathExp.upper() == 'E' or mathExp.upper() == 'У':
            return
        else:
...
def UserMane(temp=pos):
    try:
        ...
        match number:
            case 1:
                print(
                    f'Вы находитесь в меню расчета комплексных чисел.\n{menu}')
                calc_comp(count)
            case 2:
                print(
                    f'Вы находитесь в меню расчета комплексных чисел.\n{menu}')
        ...
        print('\033[33mОшибка! Введите число \033[0m\033[32m1   \033[0m\033[35m2   \033[0m\033[31m3\033[0m\033[33m для перехода в меню.\033[0m')
        return UserMane()
